...
class XRDPlotter(tk.Frame):
    ...
    def _toggle_spacing_widget(self, *args):
        ...
        self.spacing_label.grid() if is_stack_mode else self.spacing_label.grid_remove()
        self.spacing_entry.grid() if is_stack_mode else self.spacing_entry.grid_remove()
    ...
    def create_widgets(self):
        ...
        v_scrollbar.grid(row=0, column=1, sticky="ns"); self.file_listbox.config(yscrollcommand=v_scrollbar.set)
        ...
        self.xmin_var = tk.StringVar(value="30"); tk.Label(graph_settings_frame, text="横軸 最小値:").grid(row=0, column=0, sticky="w", padx=5, pady=2)
        ...
        self.xmin_entry.bind("<FocusOut>", self.schedule_update); self.xmin_entry.bind("<Return>", self.schedule_update)
        self.xmax_entry.bind("<FocusOut>", self.schedule_update); self.xmax_entry.bind("<Return>", self.schedule_update)
        ...
        container.grid(row=2, column=0, sticky="nsew"); container.rowconfigure(0, weight=1); container.columnconfigure(0, weight=1)
        canvas = tk.Canvas(container, borderwidth=0, highlightthickness=0); canvas.grid(row=0, column=0, sticky="nsew")
        scrollbar = tk.Scrollbar(container, orient="vertical", command=canvas.yview); scrollbar.grid(row=0, column=1, sticky="ns"); canvas.configure(yscrollcommand=scrollbar.set)
        self.peak_frame = tk.Frame(canvas); canvas.create_window((0, 0), window=self.peak_frame, anchor="nw"); self.peak_frame.bind("<Configure>", lambda e: canvas.configure(scrollregion=canvas.bbox("all")))
        self.peak_frame.columnconfigure(2, weight=1); self.peak_frame.columnconfigure(3, weight=1)
        ...
        for i in range(10):
            ...
            vis_var = tk.BooleanVar(value=False); tk.Checkbutton(self.peak_frame, variable=vis_var, command=self.schedule_update).grid(row=i+1, column=1); self.peak_visible_vars.append(vis_var)
            name_var = tk.StringVar(); tk.Entry(self.peak_frame, textvariable=name_var).grid(row=i+1, column=2, padx=2, pady=2, sticky="ew"); self.peak_name_vars.append(name_var); name_var.trace_add("write", self.schedule_update)
            angle_var = tk.StringVar(); tk.Entry(self.peak_frame, textvariable=angle_var).grid(row=i+1, column=3, padx=2, pady=2, sticky="ew"); self.peak_angle_vars.append(angle_var); angle_var.trace_add("write", self.schedule_update)
        ...
        create_appearance_row(appearance_frame, "目盛りフォントサイズ:", self.tick_label_fontsize_var, 3, ttk.Spinbox, from_=1, to=100)
        ...
        self.export_height_var = tk.StringVar(value="6"); tk.Label(export_frame, text="高さ (inch):").grid(row=1, column=0, sticky="w", padx=5, pady=2); tk.Entry(export_frame, textvariable=self.export_height_var).grid(row=1, column=1, sticky="ew", padx=5, pady=2)
        self.export_format_var = tk.StringVar(value="png"); tk.Label(export_frame, text="形式:").grid(row=2, column=0, sticky="w", padx=5, pady=2); ttk.Combobox(export_frame, textvariable=self.export_format_var, values=["png", "pdf"], state="readonly").grid(row=2, column=1, sticky="ew", padx=5, pady=2)
        ...
        self.canvas = FigureCanvasTkAgg(master=plot_panel); self.fig = self.canvas.figure; self.canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
    ...
    def preset_peaks(self):
        ...
        for i, peak_data in enumerate(initial_peaks):
            if i < 10:
                self.peak_name_vars[i].set(peak_data["name"])
                self.peak_angle_vars[i].set(peak_data["angle"])
    ...
    def save_figure(self):
        ...
        finally: self.fig.set_size_inches(original_size); self.canvas.draw_idle()
