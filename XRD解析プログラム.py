...
class XRDPlotter(tk.Frame):
    ...
    def build_plot_settings_tab(self, tab):
        ...
        tk.Button(
            file_button_frame, text="ファイルを選択", command=self.select_files
        ...
        h_scrollbar = tk.Scrollbar(
            listbox_frame, orient=tk.HORIZONTAL, command=self.file_listbox.xview
        )
        ...
        self.xmin_entry = tk.Entry(
            graph_settings_frame,
            textvariable=self.xmin_var,
            validate="all",
        ...
        self.xmax_entry = tk.Entry(
            graph_settings_frame,
            textvariable=self.xmax_var,
            validate="all",
            validatecommand=self.vcmd_float,
        )
        ...
        tk.Radiobutton(
            ...
            text="非表示",
            variable=self.threshold_handling_var,
            value="hide",
        ...
        self.legend_name_entry = tk.Entry(
            graph_settings_frame, textvariable=self.legend_name_var, state="disabled"
        )
        self.legend_name_entry.grid(row=5, column=1, sticky="ew", padx=5, pady=2)
        self.show_legend_check = tk.Checkbutton(
            graph_settings_frame,
            text="凡例を表示する",
            variable=self.show_legend_var,
            command=self.schedule_update,
        )
        self.show_legend_check.grid(row=6, column=0, sticky="w", padx=5, pady=2)
        ...
        self.legend_loc_combo.grid(row=6, column=1, sticky="ew", padx=5, pady=2)
        self.legend_loc_combo.bind("<<ComboboxSelected>>", self.schedule_update)
        ...
        tk.Checkbutton(
            legend_style_frame,
            text="イタリック",
            variable=self.legend_italic_var,
            command=self.schedule_update,
        ).pack(side="left")
        tk.Label(legend_style_frame, text="背景色:").pack(side="left", padx=(10, 2))
        self.legend_bg_button = tk.Button(
            legend_style_frame,
            text="■",
            width=2,
            relief=tk.SUNKEN,
        ...
        self.legend_bg_button.pack(side="left")
        self.legend_bgcolor_var.trace_add(
            "write",
            lambda *args: self.legend_bg_button.config(
                fg=self.legend_bgcolor_var.get()
            ),
        )
        self.legend_bg_button.config(fg=self.legend_bgcolor_var.get())

        tk.Checkbutton(
            graph_settings_frame,
            text="グラフを縦に並べる",
            variable=self.stack_plots_var,
            command=self._toggle_spacing_widget,
        ).grid(row=8, column=0, columnspan=2, sticky="w", padx=5, pady=2)
        self.spacing_label = tk.Label(graph_settings_frame, text="グラフの間隔 (10^n)")
        self.spacing_label.grid(row=9, column=0, sticky="w", padx=5, pady=2)
        self.spacing_entry = tk.Scale(
            graph_settings_frame,
            variable=self.plot_spacing_var,
        ...
        self.spacing_entry.grid(row=9, column=1, sticky="ew", padx=5, pady=2)
    ...
    def build_reference_peaks_tab(self, tab):
        ...
        ttk.Spinbox(
            peak_opts_frame,
            textvariable=self.peak_label_fontsize_var,
            from_=1,
            to=100,
            command=self.schedule_update,
            width=5,
        ...
        ttk.Spinbox(
            peak_opts_frame,
            textvariable=self.peak_label_y_var,
            from_=0.0,
            to=1.0,
            increment=0.05,
            command=self.schedule_update,
        ...
        ttk.Spinbox(
            peak_opts_frame,
            textvariable=self.peak_label_offset_var,
            from_=0.1,
            to=5,
            increment=0.1,
            command=self.schedule_update,
        ...
        tk.Label(self.peak_frame, text="物質名/結晶面").grid(row=0, column=2)
        ...
        for i in range(10):
            tk.Label(self.peak_frame, text=f"#{i + 1}").grid(
                row=i + 1, column=0, padx=(5, 2), pady=2, sticky="w"
            )
            vis_var = tk.BooleanVar(value=False)
            tk.Checkbutton(
                self.peak_frame, variable=vis_var, command=self.schedule_update
            ).grid(row=i + 1, column=1)
            self.peak_visible_vars.append(vis_var)
            name_var = tk.StringVar()
            tk.Entry(self.peak_frame, textvariable=name_var).grid(
                row=i + 1, column=2, padx=2, pady=2, sticky="ew"
            )
            self.peak_name_vars.append(name_var)
            name_var.trace_add("write", self.schedule_update)
            angle_var = tk.StringVar()
            tk.Entry(self.peak_frame, textvariable=angle_var).grid(
                row=i + 1, column=3, padx=2, pady=2, sticky="ew"
            )
            self.peak_angle_vars.append(angle_var)
    ...
    def build_analysis_tab(self, tab):
        analysis_frame = tk.Frame(tab, padx=10, pady=10)
        ...
        d_input_entry = tk.Entry(
            d_spacing_frame, textvariable=self.d_spacing_input_2theta_var
        )
        ...
        tk.Label(
            d_spacing_frame, textvariable=self.d_spacing_result_var, relief="sunken"
        ...
        lc_d_entry = tk.Entry(lc_frame, textvariable=self.lc_input_d_var)
        ...
        tk.Entry(hkl_frame, textvariable=self.lc_h_var, width=5).pack(side="left")
        tk.Entry(hkl_frame, textvariable=self.lc_k_var, width=5).pack(side="left")
        tk.Entry(hkl_frame, textvariable=self.lc_l_var, width=5).pack(side="left")
        ...
        tk.Label(lc_frame, textvariable=self.lc_result_var, relief="sunken").grid(
        ...
        tk.Checkbutton(
            peak_frame,
            text="有効化",
            variable=self.peak_detection_enabled_var,
            command=self.schedule_update,
        ...
        ttk.Spinbox(
            peak_frame,
            textvariable=self.peak_detection_height_var,
            from_=0,
            to=1e9,
            increment=10,
            command=self.schedule_update,
        ...
        ttk.Spinbox(
            peak_frame,
            textvariable=self.peak_detection_prominence_var,
            from_=0,
            to=1e9,
            increment=10,
            command=self.schedule_update,
        ...
        ttk.Spinbox(
            peak_frame,
            textvariable=self.peak_detection_width_var,
            from_=0,
            to=100,
            increment=0.5,
            command=self.schedule_update,
    ...
    def _create_color_picker_command(self, index):
        ...
        def command():
            color_code = colorchooser.askcolor(
                title="色を選択", initialcolor=self.peak_color_vars[index].get()
            )
            if color_code and color_code[1]:
                self.peak_color_vars[index].set(color_code[1])
                self.peak_color_buttons[index].config(fg=color_code[1])
                self.schedule_update()
    ...
    def _choose_legend_bgcolor(self):
        ...
        color = colorchooser.askcolor(
            title="凡例の背景色", initialcolor=self.legend_bgcolor_var.get()
        )
        if color and color[1]:
            self.legend_bgcolor_var.set(color[1])
            self.schedule_update()
    ...
    def calculate_d_spacing(self, *args):
        try:
            ...
            if two_theta_deg <= 0 or two_theta_deg >= 180:
                self.d_spacing_result_var.set("エラー: 2θは0-180の範囲で入力")
                return
            theta_rad = math.radians(two_theta_deg / 2.0)
            lambda_coka1 = 1.78897
            d = lambda_coka1 / (2 * math.sin(theta_rad))
            self.d_spacing_result_var.set(f"{d:.5f} Å")
        except (ValueError, TypeError):
            self.d_spacing_result_var.set("エラー: 有効な数値を入力してください")

    def copy_d_spacing(self, *args):
        try:
            result_str = self.d_spacing_result_var.get()
            d_value = result_str.split(" ")[0]
            float(d_value)
            self.lc_input_d_var.set(d_value)
        except (ValueError, IndexError):
            pass

    def calculate_lattice_constant(self, *args):
        try:
            d = float(self.lc_input_d_var.get())
            h = int(self.lc_h_var.get())
            k = int(self.lc_k_var.get())
            l = int(self.lc_l_var.get())
            if (h**2 + k**2 + l**2) == 0:
                self.lc_result_var.set("エラー: (h,k,l)は(0,0,0)にできません")
                return
            a = d * math.sqrt(h**2 + k**2 + l**2)
            self.lc_result_var.set(f"a = {a:.5f} Å")
        except (ValueError, TypeError):
            self.lc_result_var.set("エラー: 有効な数値を入力してください")
